$(function() {
	...
	*/
	...
	var changePrice = function(price) {
		...
		dollars = Math.round(Math.random()) * 2 - 1;
		//adds chance/10 to price
		price += dollars*2 + chance/2; 
	...
	for (var key in companies) {
	   ...
	   for (var prop in obj) {
			...
			*/
			if (prop === 'shares') {
				html += "<td> <a href='#' class='buy' id='" + symbol + "Buy" + "'>buy 1</a> </td> <td> <a href='#' class='sell' id='" + symbol + "Sell" + "'>sell 1</a> </td>";
			}
 	   }
 	   html += "</tr>";
			...
			for (var prop in obj) {
				...
				*/
				...
				if (prop === 'price') {
					...
					if (priceOfThisStock > roundedPrice) {
						$('#'+prop+symbol).html("<span class='negprice'>" + roundedPrice + "</span>");
					}
					else {
						$('#'+prop+symbol).html("<span class='posprice'>" + roundedPrice + "</span>");
					}
